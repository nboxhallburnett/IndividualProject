    ...
    public class RTexCameraPosition : MonoBehaviour {
        ...
        void Start () {
            ...
            for (int i = 0; i < Helper.renderDepth; i++) {
                ...
                additionalDepthRenderers[i].GetComponentInChildren<RTexCameraPosition>().enabled = false;
        ...
        void Update () {
            if (PointOfView.GetComponentInChildren<RTexCameraPosition>().inScreenView()) {
                ...
                for (int i = 0; i < Helper.renderDepth; i++) {
                    ...
                    additionalDepthRenderers[i].transform.rotation = Quaternion.Euler(rotationOffset + _defaultRot - _normalisedDefaultRot);
                }
            } else {
        ...
        public void positionPlayer (Collider player, Vector3 centre, Vector3 forward) {
            ...
            if (Vector3.Dot(distance.normalized, forward) < 0) {
                Quaternion relativeRot = Quaternion.FromToRotation(RenderPosition.forward, -PointOfView.forward);
                player.transform.rotation = relativeRot * player.transform.rotation;
                ...
                player.transform.position += relativeRot * distance;
            }
        }
        ...
        private bool inScreenView (Vector3 point) {
            Vector3 relativePos = _playerCam.WorldToScreenPoint(point);
        ...
        private bool inScreenView (Vector3[] points) {
            bool inView = false;
            foreach (Vector3 point in points) {
                if (inScreenView(point)) {
