...
namespace NEGeo {
    ...
    public class CameraRenderPosition : MonoBehaviour {
        ...
        void Start () {
            ...
            // POSITION/ROTATION IS FROM (head), OTHER SHIT FROM (eye)

            // Get relative variables for script use
            _player = GameObject.Find("Camera (head)").GetComponent<Transform>(); //FindGameObjectWithTag("MainCamera").GetComponent<Transform>();
            _playerCam = _player.GetComponent<Camera>();
            _defaultRot = Moveable.rotation.eulerAngles;
            _playerBase = GameObject.FindGameObjectWithTag("MainCamera").GetComponentInParent<Transform>();
        ...
        void LateUpdate () {
            ...
            _linkedScript.SetDepth(-1 - ((RenderPosition.position - _player.position).magnitude) / 10f);
